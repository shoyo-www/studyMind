export default function AppLoader({ subtitle = '', fullScreen = false, overlay = false }) {
  const containerClassName = fullScreen
    ? 'fixed inset-0 z-[120] flex items-center justify-center px-6 pp-app-shell'
    : overlay
      ? 'absolute inset-0 z-20 flex items-center justify-center bg-[rgba(5,8,16,0.82)] px-6 backdrop-blur-sm'
  ...
  return (
    <div
      ...
      aria-live="polite"
    >
      <div className="flex flex-col items-center gap-4 text-center pp-app-card rounded-[2rem] px-8 py-8">
        ...
        {subtitle ? (
          <div className="max-w-xs text-sm leading-relaxed pp-app-muted">
            {subtitle}
          </div>
        ) : null}
        ...
        <div className="flex gap-1.5">
          {[0, 1, 2].map((index) => (
            <div
              ...
              className="h-1.5 w-1.5 rounded-full bg-[var(--pp-coral)] opacity-40 animate-[preppal-loader-bounce_1s_infinite]"
            />
          ))}
        </div>
