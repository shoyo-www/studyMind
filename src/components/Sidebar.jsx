...
export default function Sidebar({ screen, setScreen, user, profile }) {
  const { t, lang } = useT()
  ...
  return (
    <aside className="w-56 shrink-0 bg-white border-r border-zinc-100 flex flex-col h-full">
          ...
          return (
            <div key={item.id}>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-6 py-2.5 text-sm transition-all duration-150 text-left
                  ${screen === item.id
                    ? 'text-violet-600 bg-violet-50 border-r-2 border-violet-500 font-medium'
                    : 'text-zinc-400 hover:text-zinc-700 hover:bg-zinc-50'
                  }`}
              >
                <span className={screen === item.id ? 'text-violet-600' : 'text-zinc-300'}>
                  {icons[item.id]}
                </span>
                {t(`nav.${item.id}`)}
              </button>
      ...
      <div className="px-5 py-4 border-t border-zinc-100 flex items-center gap-3">
        <div className="w-8 h-8 rounded-full bg-violet-100 flex items-center justify-center text-xs font-semibold text-violet-700 shrink-0">{avatarLetter || 'S'}</div>
